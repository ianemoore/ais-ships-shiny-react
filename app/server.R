        ...
        grid(
          ...
          desc = div(
            ...
            div(
              ...
              p(
                'Style the application using SCSS syntax with {sass} enabled preprocessing to machine readable css code using the {sass} package at the Global context level in production and at the server context level during development'
              )
          ...
          extra = div(
            ...
            div(
              ...
              p(
                'Smooth map experience, with special attention payed to not interupting a users set zoom unless the two points were outside of their current viewport.'
              ),
              
              p(
                'Effiecient Reactive flow throughout the application. Fine control over when to process React and Scss code allows for live code updates to core frontend components.'
              ),
              
              p(
                'Advanced JS -> Shiny -> JS communication can be found in the main search functionality to populate React based outputs that can be clicked to register \'select vessel\' event back in Shiny. The search input is meant to pair efficient R data filtering capabilities with Reacts powerful virtual DOM capabilities.'
              )
            ...
            div(
              ...
              p(
                'I had a difficult time implementing normal testing best practices in this app. I don\'t quite know for sure but my suspicion is that using React in this app has somehow interupted the way that {testthat} processes the shiny code.'
              ),
              
              p(
                'Rendering React components requires that the target rendering div exists in the DOM before the React app is called. This required using a dedicated shiny module and a uiOutput to create a slight delay between the main UI being rendered and the React javascript being processed.'
              )
