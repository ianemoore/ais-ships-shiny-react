...
build_base_map <-
  function(map_id,
    ...
    leaf <- leaflet(map_id,
                    options = leafletOptions(zoomControl = zoom_ctl0,
                                             minZoom = min_zoom0)) %>%
      
      addTiles(urlTemplate = tile_url1,
               group = "Sat") %>%
      ...
      addTiles(urlTemplate = tile_url2,
               group = "Topo") %>%
      
      addLayersControl(baseGroups = c("Sat", "Topo", "Roads"),
                       position = "topright")
...
babel_to_js <- function(react_raw) {
  ...
  for (b in 1:length(b_lines)) {
    ...
    for (v in 1:length(wv)) {
      ...
      if (substr(chunk, 1, 3) == '-->') {
        chunk <- substr(chunk, 4, nchar(chunk))
      }
...
sem_animBtn <-
  function(input_id,
    ...
    tag0 <- div(
      ...
      div(class = paste0("hidden content"),
          c_hide)
